...
func serveStatic(t *testing.T, distDir string, path string) *httptest.ResponseRecorder {
	t.Helper()
	gin.SetMode(gin.TestMode)
	router := gin.New()
	router.NoRoute(NewStaticHandler(distDir).Serve)
...
func TestStaticServeFallbackIndex(t *testing.T) {
	...
	w := serveStatic(t, dir, "/some/spa/route")
	if w.Code != http.StatusOK {
		t.Fatalf("status = %d, want 200", w.Code)
	}
